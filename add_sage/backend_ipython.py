...
class BackendIPythonNotebook(BackendIPython):
    ...
    def displayhook(self, plain_text, rich_output):
        ...
        if isinstance(rich_output, OutputPlainText):
            ...
            import getpass
            jsmol = JSMolHtml(rich_output, height=500, path_to_jsmol=os.path.join('/user', getpass.getuser(),'static','jsmol'))
            #jsmol = JSMolHtml(rich_output, height=500, path_to_jsmol=os.path.join('/static','jsmol'))
